...
function transform(html) {
    ...
    output = output.replace('</body>', '    <script src="/demo-form.js" defer></script>\n</body>');
    return output;
}
...
for (const route of routes) {
    ...
    if (/localhost|preview\.php|<\?php/i.test(html)) throw new Error(`${route.name}: unsafe preview reference remains`);
